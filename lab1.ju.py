...
from sklearn.preprocessing import KBinsDiscretizer, OneHotEncoder, StandardScaler, RobustScaler, OrdinalEncoder
...
df = cast(pd.DataFrame, df[~df["RiskScore"].isna()])
...
def preprocess(X_in: pd.DataFrame) -> pd.DataFrame:
    ...
    X["SavingsToLoanRatio"] = X["SavingsAccountBalance"] / (X["LoanAmount"] +
                                                            1)
    X["CreditHistoryInteraction"] = X["LengthOfCreditHistory"] * X[
        "PaymentHistory"]
    ...
    log_candidates = [
        "MonthlyIncome", "LoanAmount", "SavingsAccountBalance",
...
categorical = X.select_dtypes(include=["object"]).columns
numerical = [
    x for x in X.select_dtypes(include=[np.number]).columns if x not in binned
]
...
binned_transformer = Pipeline(steps=[(
    "bin",
    KBinsDiscretizer(n_bins=20, encode="onehot-dense", strategy="quantile"))])

preprocessor = ColumnTransformer(transformers=[(
    "num", numerical_transformer,
    numerical), ("cat", categorical_transformer,
                 categorical), ("bin", binned_transformer, binned)])
...
X_train, X_test, y_train, y_test = train_test_split(X_eng,
                                                    y,
                                                    test_size=0.3,
                                                    random_state=42,
                                                    stratify=y_bins)
...
class Linear:
    ...
    def _fit_gradient(self, X, y, learning_rate, n_iterations):
        ...
        for i in range(n_iterations):
            ...
            if np.any(np.isnan(self.weights)) or np.any(np.isinf(
                    self.weights)):
                raise RuntimeError(
                    f"Warning: NaN/Inf detected at iteration {i}\n" +
    ...
    def _fit_stoch(self, X, y, learning_rate, n_iterations):
        ...
        for _ in range(n_iterations):
            ...
            for i in range(h):
                ...
                self.weights -= learning_rate * gradient
                if np.any(np.isnan(self.weights)) or np.any(
                        np.isinf(self.weights)):
                    raise RuntimeError(
                        f"Warning: NaN/Inf detected at iteration {i}\n" +
                        f"Gradient norm: {np.linalg.norm(gradient)}")
...
X_train, X_test, y_train, y_test = train_test_split(X2.to_numpy(),
                                                    y2.to_numpy(),
                                                    test_size=0.33,
                                                    random_state=42)
